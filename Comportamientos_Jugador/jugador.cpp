	...
	if (sensores.nivel != 4) {
		if (!hayPlan) {
			...
			switch (sensores.nivel) {
				case 0: plan = anchuraSoloJugador_V2 (c_state, goal, mapaResultado);
					break;
			...
			if (plan.size() > 0) {
				...
				else if (sensores.nivel == 1) visualizarPlan(c_state1, plan);
...
void ComportamientoJugador::visualizarPlan (const stateN1 &st, const list<Action> &plan) {
	anularMatriz(mapaConPlan);
	stateN1 cst = st;
...
/* Encuentra si el elemento item está en la lista */
bool ComportamientoJugador::Find (const stateN2 &item, const list<stateN2> &lista) {
	auto it = lista.begin();
	while ( it != lista.end() and !( (*it) == item ) ) {
		it++;
	}
...
bool ComportamientoJugador::Find (const stateN2 &item, const list<nodeN2> &lista) {
	auto it = lista.begin();
	while ( it != lista.end() and !( it->st == item ) ) {
		it++;
	}
...
bool ComportamientoJugador::jugadorVeColaborador(const ubicacion & jug, const ubicacion & clb){
	
	switch (jug.brujula)
	{
	case norte:

		if ((jug.f - 1) == clb.f and (jug.c - 1) == clb.c)
			return true;
		if ((jug.f - 1) == clb.f and (jug.c) 	 == clb.c)
			return true;
		if ((jug.f - 1) == clb.f and (jug.c + 1) == clb.c)
			return true;
		if ((jug.f - 2) == clb.f and (jug.c - 2) == clb.c)
			return true;
		if ((jug.f - 2) == clb.f and (jug.c - 1) == clb.c)
			return true;
		if ((jug.f - 2) == clb.f and (jug.c) 	 == clb.c)
			return true;
		if ((jug.f - 2) == clb.f and (jug.c + 1) == clb.c)
			return true;
		if ((jug.f - 2) == clb.f and (jug.c + 2) == clb.c)
			return true;
		if ((jug.f - 3) == clb.f and (jug.c - 3) == clb.c)
			return true;
		if ((jug.f - 3) == clb.f and (jug.c - 2) == clb.c)
			return true;
		if ((jug.f - 3) == clb.f and (jug.c - 1) == clb.c)
			return true;
		if ((jug.f - 3) == clb.f and (jug.c) 	 == clb.c)
			return true;
		if ((jug.f - 3) == clb.f and (jug.c + 1) == clb.c)
			return true;
		if ((jug.f - 3) == clb.f and (jug.c + 2) == clb.c)
	...
	case este:
		...
		if ((jug.f - 1) == clb.f and (jug.c + 1) == clb.c)
			return true;
		if ((jug.f) 	== clb.f and (jug.c + 1) == clb.c)
			return true;
		if ((jug.f + 1) == clb.f and (jug.c + 1) == clb.c)
			return true;
		...
		if ((jug.f - 2) == clb.f and (jug.c + 2) == clb.c)
			return true;
		if ((jug.f - 1) == clb.f and (jug.c + 2) == clb.c)
			return true;
		if ((jug.f) 	== clb.f and (jug.c + 2) == clb.c)
			return true;
		if ((jug.f + 1) == clb.f and (jug.c + 2) == clb.c)
			return true;
		if ((jug.f + 2) == clb.f and (jug.c + 2) == clb.c)
		...
		if ((jug.f - 3) == clb.f and (jug.c + 3) == clb.c)
			return true;
		if ((jug.f - 2) == clb.f and (jug.c + 3) == clb.c)
			return true;
		if ((jug.f - 1) == clb.f and (jug.c + 3) == clb.c)
			return true;
		if ((jug.f) 	== clb.f and (jug.c + 3) == clb.c)
			return true;
		if ((jug.f + 1) == clb.f and (jug.c + 3) == clb.c)
			return true;
		if ((jug.f + 2) == clb.f and (jug.c + 3) == clb.c)
													...
													const vector<vector<unsigned char>> &mapa)
{
	...
	while (!frontier.empty() and !solutionFound)
	{
		...
		if (!frontier.empty())
		{
			...
			// si el nodo hijo no está en explorados y tampoco en frontier
			else if (!Find(child_walk.st, explored) and itw == frontier.end())
			{
				child_walk.secuencia.push_back(actWALK);
				insertarEnOrden(frontier, child_walk); // inserta en explorados
			}
		...
		// Si frontier estaba vacío y el nodo hijo no ha sido explorado todavía
		else if (!Find(child_walk.st, explored))
		{
			child_walk.secuencia.push_back(actWALK);
			frontier.push_back(child_walk); // añade el nodo hijo con la accion actwalk en su secuencia
		}
		...
		if (!solutionFound) {
			...
			if (!frontier.empty())
			{
				...
				// si el nodo hijo no está en explorados y tampoco en frontier
				else if (!Find(child_run.st, explored) and (itl == frontier.end()))
				{
					child_run.secuencia.push_back(actRUN);
					insertarEnOrden(frontier, child_run); // inserta en explorados
				}
				// si el nodo hijo ya está en frontier pero tiene un coste mayor que el nuevo coste calculado
			...
			// Si frontier estaba vacío y el nodo hijo no ha sido explorado todavía
			else if (!Find(child_run.st, explored))
			{
				child_run.secuencia.push_back(actRUN); // añade el nodo hijo con la accion actTURN_L en su secuencia
				frontier.push_back(child_run);
			}
		}
		...
		if (!solutionFound){
			...
			if (!frontier.empty())
			{
				...
				// si el nodo hijo no está en explorados y tampoco en frontier
				if (!Find(child_turnl.st, explored) and (itl == frontier.end()))
				{
					child_turnl.secuencia.push_back(actTURN_L);
					insertarEnOrden(frontier, child_turnl); // inserta en explorados
				}
			...
			// Si frontier estaba vacío y el nodo hijo no ha sido explorado todavía
			else if (!Find(child_turnl.st, explored))
			{
				child_turnl.secuencia.push_back(actTURN_L); // añade el nodo hijo con la accion actTURN_L en su secuencia
				frontier.push_back(child_turnl);
			}
		...
		if (!solutionFound) {
			...
			if (!frontier.empty())
			{
				...
				// si el nodo hijo no está en explorados y tampoco en frontier
				if (!Find(child_turnsr.st, explored) and (itsr == frontier.end()))
				{
					child_turnsr.secuencia.push_back(actTURN_SR);
					insertarEnOrden(frontier, child_turnsr); // inserta en explorados
				}
			...
			// Si frontier estaba vacío y el nodo hijo no ha sido explorado todavía
			else if (!Find(child_turnsr.st, explored))
			{
				child_turnsr.secuencia.push_back(actTURN_SR); // añade el nodo hijo con la accion actTURN_SR en su secuencia
				frontier.push_back(child_turnsr);
			}
		}
